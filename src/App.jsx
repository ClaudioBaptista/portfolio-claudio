...
const App = () => {
  ...
  const techCategories = {
    ...
    "Core Engineering": ["React", "Node.js", "Python", "SQL", "Bases Distribuídas"]
  };
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Mensagem enviada com sucesso! A nossa equipa entrará em contacto.", {
          style: { background: theme === 'dark' ? '#39FF14' : '#15803d', color: theme === 'dark' ? '#0A192F' : '#ffffff', fontWeight: 'bold' },
          iconTheme: { primary: theme === 'dark' ? '#0A192F' : '#ffffff', secondary: theme === 'dark' ? '#39FF14' : '#15803d' },
        });
        form.reset();
      } else {
        toast.error("Ocorreu um problema ao enviar. Tente novamente.", { style: { background: '#ef4444', color: '#fff', fontWeight: 'bold' }});
      }
    } catch (error) {
      toast.error("Erro de ligação. Verifique a sua internet.", { style: { background: '#ef4444', color: '#fff', fontWeight: 'bold' }});
    }
  };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ease-in-out ${theme === 'dark' ? 'bg-background text-gray-300' : 'bg-gray-50 text-gray-700'} selection:bg-neon selection:text-background font-sans relative overflow-x-hidden`}>
      ...
      <nav className={`fixed top-0 w-full z-50 backdrop-blur-md border-b transition-colors duration-300 ${theme === 'dark' ? 'bg-background/90 border-neon/20' : 'bg-white/90 border-black/5'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 text-sm font-semibold tracking-wider uppercase items-center">
            ...
            <NavLink href="#founder" theme={theme} neonClass={neonTextClass}>Liderança</NavLink>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`absolute top-20 left-0 w-full border-b p-6 flex flex-col gap-6 md:hidden shadow-2xl ${theme === 'dark' ? 'bg-background border-neon/20' : 'bg-white border-gray-100'}`}>
              ...
              <MobileLink href="#founder" onClick={() => setIsMenuOpen(false)} theme={theme} neonClass={neonTextClass}>Liderança</MobileLink>
      ...
      <section className="relative min-h-screen flex flex-col justify-center items-center px-6 text-center z-10 pt-20">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative max-w-5xl">
          ...
          <h2 className="text-2xl md:text-4xl font-light mb-8 h-12">
            <span className={`transition-colors duration-300 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Lógica em{' '}</span>
            <span className={`${neonTextClass} font-bold inline-block`}>
               <AnimatePresence mode="wait">
                 <motion.span key={currentWord} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                   {words[currentWord]}
                 </motion.span>
               </AnimatePresence>
            </span>
      ...
      <div className={`py-6 border-y relative flex overflow-hidden ${theme === 'dark' ? 'bg-[#0a1120] border-white/5' : 'bg-gray-100 border-gray-200'}`}>
        ...
        <motion.div className="flex gap-16 items-center whitespace-nowrap min-w-max px-8" animate={{ x: ["0%", "-50%"] }} transition={{ repeat: Infinity, ease: "linear", duration: 30 }}>
          {[...Array(2)].map((_, i) => (
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  <AnimatePresence mode='popLayout'>
                    {getDisplayTechs().map((tag) => (
                      <motion.span key={tag} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }} className={`px-4 py-2 border rounded-full text-xs font-bold transition-all cursor-default ${theme === 'dark' ? 'bg-[#0f1a2e] border-neon/20 text-gray-300 hover:border-neon hover:text-neon hover:shadow-neon' : 'bg-gray-50 border-gray-200 text-gray-700 hover:border-green-600 hover:text-green-700 hover:bg-white'}`}>
                        {tag}
                      </motion.span>
                    ))}
                  </AnimatePresence>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-card border-white/5' : 'bg-white border-gray-100 shadow-sm'}`}>
                 <div className={`mb-3 ${neonTextClass}`}><Code2 size={24} /></div>
                 <h4 className={`font-bold text-sm mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Sistemas & IA</h4>
                 <p className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>Arquitetura, Desenvolvimento e Automação.</p>
               </div>
               <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-card border-white/5' : 'bg-white border-gray-100 shadow-sm'}`}>
                 <div className={`mb-3 ${neonTextClass}`}><Globe size={24} /></div>
                 <h4 className={`font-bold text-sm mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Supply Chain</h4>
                 <p className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>Otimização de rotas e fluxo logístico.</p>
               </div>
            </div>
          </motion.div>
      ...
      {/* SERVIÇOS */}
      <section id="services" className={`py-24 px-6 border-t relative z-10 ${theme === 'dark' ? 'bg-background border-white/5' : 'bg-white border-gray-200'}`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
            <ServiceCard theme={theme} icon={<Brain size={36} />} title="IA Aplicada à Logística" desc="Modelos preditivos, automação de fluxo e redes neuronais focadas no setor de transportes." features={["Previsão de Demanda", "Análise de Dados Históricos", "Machine Learning"]} borderColor={theme === 'dark' ? "hover:border-neon" : "hover:border-green-600"} iconColor={theme === 'dark' ? "text-neon" : "text-green-600"} />
            <ServiceCard theme={theme} icon={<Truck size={36} />} title="Roteirização Inteligente" desc="Algoritmos matemáticos complexos para otimização de rotas e redução de custos com frotas." features={["Algoritmos de Roteamento", "Redução de Tempo Ocioso", "Otimização de Carga"]} borderColor={theme === 'dark' ? 'hover:border-white' : 'hover:border-gray-800'} iconColor={theme === 'dark' ? 'text-white' : 'text-gray-800'} />
            <ServiceCard theme={theme} icon={<Globe size={36} />} title="Supply Chain Visibility" desc="Monitorização global, processamento de dados em tempo real e integração com TMS/WMS." features={["Dashboards em Tempo Real", "Integração via API (ERP/TMS)", "Alertas Automatizados"]} borderColor="hover:border-purple" iconColor="text-purple" />
            <ServiceCard theme={theme} icon={<Code2 size={36} />} title="Sistemas Escaláveis" desc="Arquitetura de software de alta performance para suportar operações logísticas críticas." features={["Arquitetura Cloud (AWS/Azure)", "Bases de Dados Distribuídas", "Segurança e Conformidade"]} borderColor="hover:border-blue-500" iconColor="text-blue-500" />
          </div>
        </div>
      </section>
      ...
      <section id="contact" className={`py-24 px-6 relative z-10 border-t transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0a1120] border-white/5' : 'bg-gray-50 border-gray-200'}`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <h2 className={`text-4xl md:text-5xl font-extrabold mb-6 leading-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              Vamos otimizar a sua <br/><span className={neonTextClass}>operação.</span>
            </h2>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-500">
              ...
              <li><FooterLink href="#founder" theme={theme} neonClass={neonTextClass}>Liderança</FooterLink></li>
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} onClick={scrollToTop} className={`fixed bottom-8 right-8 p-3 rounded-full z-50 transition-colors ${theme === 'dark' ? 'bg-neon text-background hover:bg-white shadow-neon' : 'bg-gray-900 text-white hover:bg-green-600 shadow-lg'}`}>
            <ArrowUp size={24} />
          </motion.button>
        )}
      </AnimatePresence>
